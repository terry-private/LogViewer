...
- 最低バージョンを意図的に引き上げるプルリクエストでは、
  `README.md`、この文書、検証手順と、宣言対象が変わる場合は
  `Package.swift`を同時に更新する。

`Package.swift`のSwift tools 6.2だけではXcodeのminor下限を表現できないため、
Xcode 26.2という下限はこの文書で宣言し、継続的検証で確認する。
...
秘匿化と本番環境でのアクセス制御は
[READMEのプライバシーと書き出し](../README.md#プライバシーと書き出し)を参照する。

## セマンティックバージョニング

- patchは互換性を保つ不具合修正と文書修正に使う
- minorは既存利用側を壊さないAPIと機能の追加に使う
- majorは公開APIの削除、必須引数化、意味変更などの破壊的変更に使う
- 1.xで非推奨化するときは代替APIと移行手順を同時に示し、削除は次のmajorで行う
- JSON v1の既存フィールドの意味と型は1.xで維持する

公開APIの移行手順は[MIGRATION](MIGRATION.md)、版ごとの利用者向け変更は
[CHANGELOG](../CHANGELOG.md)を正本とする。
