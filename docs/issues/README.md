...
- `in-progress`: 作業中
- `done`: default branchへマージされ、必要な検証を完了した

GitHub Issueを進行状態の正本とし、この台帳は設計本文とGitHub番号の
ローカルミラーとして扱う。`done`へ変更するのは、変更がdefault branchへ
マージされ、必要な検証が完了した後とする。
...
| LV-001 | [#2](https://github.com/terry-private/LogViewer/issues/2) | supported environments and build matrix | - | in-progress |
| LV-002 | [#3](https://github.com/terry-private/LogViewer/issues/3) | characterization tests and CI-ready commands | LV-001 | in-progress |
